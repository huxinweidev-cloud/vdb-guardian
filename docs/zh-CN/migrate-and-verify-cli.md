...
- `--pgvector-id-column`: `id`
- `--pgvector-vector-column`: `embedding`
- `--pgvector-write-mode`: `batch-upsert`。支持 `batch-upsert`、`copy` 和 `auto`；该值会在构建验证 artifact 前传递给迁移步骤。
...
`migrate-and-verify` 接受与 `migrate` 相同的 `--pgvector-write-mode batch-upsert|copy|auto` 参数，并在构建 fingerprint 和 full-record verification artifacts 之前把它传递给内部 migration 步骤：
...
- `batch-upsert` 是默认值，也是最安全的 legacy 行为。
- `copy` 对每个批次使用 staging table、事务、PostgreSQL `COPY` 和 merge/upsert 写入目标表。显式 `copy` 不会 fallback；COPY、schema、validation 或 context 失败都会让该批次失败。
- `auto` 先尝试 COPY，仅对可恢复的 COPY execution failure fallback 到 `batch-upsert`。classifier 有意保守，不会对 validation/schema/context 失败 fallback。

只有批次写入成功后才会推进 checkpoint。Markdown 和 diagnostic JSON reports 会包含 `write_mode_requested`、`write_mode_used`、`copy_batches`、`batch_upsert_batches` 和 `copy_fallbacks`。当 completed batches 同时使用 COPY 和 batch upsert 时，`write_mode_used` 为 `mixed`，例如发生 `auto` fallback 的运行。
...
- 可选 `--pgvector-write-mode batch-upsert|copy|auto` 透传，并在 Markdown 与 diagnostic JSON reports 中记录 write-mode metrics。
...
为了达成严苛的生产环境数据一致性，未来的迭代还需要加入源端 streaming cursor 和 stale-row reconciliation 运维 runbook。
