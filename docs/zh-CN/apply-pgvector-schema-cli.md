...
可通过 `inspect-pgvector-schema` 和 `compare-applied-schema` 验证实际 PostgreSQL schema。`inspect-pgvector-schema` 会 inventory live schema metadata，`compare-applied-schema` 会在记录迁移前检测 planned-vs-live drift。
