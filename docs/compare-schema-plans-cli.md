...
The command does not connect to Milvus, does not connect to PostgreSQL, and does not execute SQL. After this comparison passes, use `vdbg apply-pgvector-schema` to dry-run or execute the validated schema DDL, then use `vdbg inspect-pgvector-schema` and `vdbg compare-applied-schema` to inventory and validate the live target schema.
