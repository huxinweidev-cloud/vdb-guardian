...
Use `inspect-pgvector-schema` and `compare-applied-schema` to validate live PostgreSQL schema after execution. `inspect-pgvector-schema` inventories live schema metadata, and `compare-applied-schema` detects planned-vs-live drift before record migration.
